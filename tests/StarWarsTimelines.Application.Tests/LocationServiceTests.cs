...
public sealed class LocationServiceTests
{
    ...
    {
        await Assert.ThrowsAsync<BadRequestException>(() => _service.CreateAsync(new CreateLocationRequest(name!)));
    ...
    public async Task UpdateAsync_WithInvalidName_Throws()
    {
        ...
        await Assert.ThrowsAsync<BadRequestException>(() => _service.UpdateAsync(item.Id, new UpdateLocationRequest("   ")));
